...
동일 변경안에 대한 사용자 승인을 확인하고 승인된 항목만 적용한다. 이미 받은 승인은 반복 요청하지 않는다. 위임이 허용되고 유용하면 아래 템플릿을 사용하고, 아니면 본체가 같은 계약으로 수행한다.
...
10. MCP 서버·플러그인·훅처럼 **런타임 로딩에 영향을 주는 변경**은 파일 재독으로 끝내지 말고,
    ...
    파일 적용 결과와 "동작 미검증(Codex)"을 분리해 보고한다. 파일 확인을 동작 확인으로 격상하지 않는다.
